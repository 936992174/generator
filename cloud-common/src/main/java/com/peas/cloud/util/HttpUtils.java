...
public class HttpUtils {
	 ...
	 */
	public static com.peas.cloud.util.HttpResponse httpPostForm(String url,Map<String,String> params, Map<String,String> headers,String encode){
		com.peas.cloud.util.HttpResponse response = new com.peas.cloud.util.HttpResponse();
		if(encode == null){
			encode = "utf-8";
		}
	...
	public static String doPostHttp(String apiUrl, Map<String, String> headers, Map<String, Object> params) {
		...
		try {
			...
			if(Objects.nonNull(response) && Objects.nonNull(response.getStatusLine())){
				log.info("请求返回状态码:{}",response.getStatusLine().getStatusCode());
			}
			if (response != null) {
				try {
					EntityUtils.consume(response.getEntity());
